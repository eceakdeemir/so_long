...
int map_name_check(char *map_name)
{
    ...
    if (i < 4)
        return(0);
    if (map_name[i - 4] != '.' || map_name[i - 3] != 'b' || map_name[i - 2] != 'e' || map_name[i - 1] != 'r')
        return(0);
    return (1);
}
...
int check_exit(t_prog *prog)
{
    ...
    if(prog->fake_map[prog->exit->exit_y + 1][prog->exit->exit_x] == 'A' && collection_number == 0)
        return (1);
    else if(prog->fake_map[prog->exit->exit_y][prog->exit->exit_x + 1] == 'A' && collection_number == 0)
        return (1);
    else if(prog->fake_map[prog->exit->exit_y - 1][prog->exit->exit_x] == 'A' && collection_number == 0)
        return (1);
    else if(prog->fake_map[prog->exit->exit_y][prog->exit->exit_x - 1] == 'A' && collection_number == 0)
        return (1);
    return (0);
